 ...
 * @example
 * setValues({ id: 'status', value: 1 })
 * or
 * setValues({ status: 1, options: [1, 2] }, 'sample');
 * // => {
 * form: {
 *    sample: {
 *      status: 1,
...
/**
 * Keep form fields but set undefined every keys
 */
export const reset = (form?: string): ActionType =>
({
  type: 'FORM_CLEAR',
  form,
})
