	...
	function()
	{
		return {
			...
			langs: {
				en: {
					"video": "Embed",
					"video-html-code": "Embed Code or Youtube/Vimeo Link"
				}
			},
			getTemplate: function()
			{
				return String()
				+ '<div class="modal-section" id="redactor-modal-video">'
					+ '<section>'
			...
			show: function()
			{
				...
				this.modal.setTitle(this.lang.get('video'));

				// action button
				// this.modal.getActionButton().text(this.lang.get('insert')).on('click', this.video.insert);
				this.modal.show();

				$('#redactor-modal-button-action').on('click', this.video.insert);
				// $('#redactor-modal-button-cancel').on('click', this.modal.close);
